...
import { collectTeams, loadFeedCoupons } from "@/lib/bros/loaders";
...
export default async function BroBoardPage({ searchParams }: PageProps) {
  ...
  const tab: FeedTab = requestedTab === "following" && !isAuth ? "all" : requestedTab;

  const coupons = await loadFeedCoupons({
    followerId: tab === "following" && isAuth ? requester.user_id : null,
    limit: 30,
  });
...
function EmptyState({ tab, isAuth }: { tab: FeedTab; isAuth: boolean }) {
    ...
    return (
      <div className="glass-strong rounded-3xl p-8 text-center space-y-3">
        ...
        <p className="text-sm text-foreground/55 max-w-prose mx-auto">
          Switch to <span className="text-foreground/85">All bros</span> to find
          a few who keep cashing.
        </p>
        <Link
          href="/bros"
  ...
  return (
    <div className="glass-strong rounded-3xl p-8 text-center space-y-3">
      ...
      <p className="text-sm text-foreground/55 max-w-prose mx-auto">
        {isAuth
          ? "Build a coupon, play it, and hit Share to Bro Board on your History page."
          : "Sign in, build a coupon, and share it to kick things off."}
      </p>
      <Link
